...
static inline void encode_config_ini(zc_ini_config_t *config)
{
...
static inline int send_config(struct sockaddr *addr, pb_size_t which_config)
{
	...
	case ZC_REQUEST_GET_CONFIG_INI_TAG:
		...
		encode_config_ini(&config->config.ini);
		break;
	default:
		error = -EINVAL;
		goto send_error;
...
static inline int apply_config_ini(zc_ini_config_t *config)
{
...
static inline int apply_config(struct sockaddr *addr, zc_config_t *config)
{
	...
	case ZC_CONFIG_INI_TAG:
		error = apply_config_ini(&config->config.ini);
		break;
	default:
		error = -ENOTSUP;
		goto send_error;
